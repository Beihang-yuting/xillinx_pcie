//=============================================================================
// 文件名: tb_rc_multi_ep.sv
// 描述: Xilinx PCIe BFM 多 agent Testbench (1 RC + 4 EP, 无 DUT)
//
// 功能：
//   1. 生成 250MHz 系统时钟（4ns 周期）
//   2. 产生 10+1 周期低电平有效复位
//   3. 实例化 1 套 RC + 4 套 EP 的 xilinx_pcie_if / xilinx_pcie_cfg_if
...
//   6. 启动 run_test()
//
// 本 TB 同时服务两个回归测试（均在 filelist_rc_multi_ep.f 中）：
//   - xilinx_pcie_multi_ep_test     : num_rc=0, num_ep=4（纯 EP）
//   - xilinx_pcie_rc_multi_ep_test  : num_rc=1, num_ep=4（1 RC + 多 EP）
//   故支持 num_rc∈{0,1}, num_ep∈{0..4}。RC 例化即使 num_rc=0 也无害
//   （env 按 cfg.num_rc 决定是否构建 RC agent，多余的线上 if 保持空闲）。
//=============================================================================
...
module tb_rc_multi_ep;
    ...
    xilinx_pcie_cfg_if ep3_cfg_if (.clk(clk), .rst_n(rst_n));
    ...
    assign ep3_if.rq_tready = 1'b1;
    assign ep3_if.cc_tready = 1'b1;
    ...
    `XILINX_PCIE_WIRE_EP(3, ep3_if, ep3_cfg_if, clk, rst_n)
    ...
    initial begin
        if ($test$plusargs("DUMP_WAVES")) begin
            $dumpfile("tb_rc_multi_ep.vcd");
            $dumpvars(0, tb_rc_multi_ep);
            $display("[tb_rc_multi_ep] 波形录制已启动 -> tb_rc_multi_ep.vcd");
        end
    end
    ...
    initial begin
        #10ms;
        $display("[tb_rc_multi_ep] 错误：仿真超时（10ms），强制结束");
        $finish(2);
    end

endmodule : tb_rc_multi_ep
