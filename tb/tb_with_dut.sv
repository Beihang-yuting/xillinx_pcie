...
module tb_with_dut;
    ...
    // 参数定义（与真实 IP 配置保持一致）
    //=========================================================================
    localparam int DATA_WIDTH     = 256;   // AXI-Stream 数据位宽
    localparam int RQ_TUSER_WIDTH = 137;   // RQ 通道 tuser 位宽
    localparam int RC_TUSER_WIDTH = 161;   // RC 通道 tuser 位宽
    localparam int CQ_TUSER_WIDTH = 183;   // CQ 通道 tuser 位宽
    localparam int CC_TUSER_WIDTH = 81;    // CC 通道 tuser 位宽
    localparam int KEEP_WIDTH     = DATA_WIDTH / 32;  // per-DW keep 位宽
    ...
    */
    ...
    // EP-RQ：EP BFM MASTER，向 IP 发送请求
    axis_if #(
        .TDATA_WIDTH (DATA_WIDTH),
        .TUSER_WIDTH (RQ_TUSER_WIDTH),
        .HAS_TKEEP   (1),
        .HAS_TLAST   (1)
    ) ep_rq_if (.aclk(user_clk), .aresetn(user_rst_n));

    // EP-RC：EP BFM SLAVE，接收 IP 返回的完成
    axis_if #(
        .TDATA_WIDTH (DATA_WIDTH),
        .TUSER_WIDTH (RC_TUSER_WIDTH),
        .HAS_TKEEP   (1),
        .HAS_TLAST   (1)
    ) ep_rc_if (.aclk(user_clk), .aresetn(user_rst_n));

    // EP-CQ：EP BFM SLAVE，接收 IP 转发的请求
    axis_if #(
        .TDATA_WIDTH (DATA_WIDTH),
        .TUSER_WIDTH (CQ_TUSER_WIDTH),
        .HAS_TKEEP   (1),
        .HAS_TLAST   (1)
    ) ep_cq_if (.aclk(user_clk), .aresetn(user_rst_n));

    // EP-CC：EP BFM MASTER，向 IP 发送完成
    axis_if #(
        .TDATA_WIDTH (DATA_WIDTH),
        .TUSER_WIDTH (CC_TUSER_WIDTH),
        .HAS_TKEEP   (1),
        .HAS_TLAST   (1)
    ) ep_cc_if (.aclk(user_clk), .aresetn(user_rst_n));

    //=========================================================================
    // axis_if <-> ep_if 信号桥接（与 tb_top.sv EP 侧相同）
    //=========================================================================

    // EP-RQ：MASTER 方向（ep_rq_if 驱动 ep_if.rq_*）
    ...
    // EP-RC：SLAVE 方向（ep_if.rc_* 驱动 ep_rc_if）
    ...
    // EP-CQ：SLAVE 方向（ep_if.cq_* 驱动 ep_cq_if）
    ...
    // EP-CC：MASTER 方向（ep_cc_if 驱动 ep_if.cc_*）
